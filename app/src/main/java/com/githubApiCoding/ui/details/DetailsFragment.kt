...
class DetailsFragment : Fragment() {
    ...
    private fun onStateChanged(state: ViewModelState?) {
        when (state) {
            ...
            is StateError -> showError(state.throwable, R.string.error_message_details, { progressView.empty() })
            else -> progressView.empty()
        }
    }

    private fun setProfileInfo(state: ProfileLoaded) {
        if (state.isName && state.isRepos) progressView.empty() // hide progress when all data loaded
        val user = state.user
